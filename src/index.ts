...
class Parameters {
  ...
  public present() {
    ...
    obj.difficulty = this.options.difficulty !== "Mix" ? this.options.difficulty.toLowerCase() : undefined;

    if (
      this.options.type !== "Multiple Choice" && this.options.type !== "Mix"
    ) {
      obj.type = "boolean"
    ...
    obj.category = this.options.category[0] === "0" ?
      undefined :
...
class main {
  ...
  async start() {
    ...
    if (!this.playerName) {
      this.playerName = (await new Question(
        "What is your name?",
        "input",
        undefined,
        "Anonymous"
    ...
    if (Array.isArray(questions)) {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (answer === questions[i].choices.correct) {
          ...
          this.score--;
          console.log(`❌ ${this.playerName}, your score decreased to ${this.score}`)
        } else {
